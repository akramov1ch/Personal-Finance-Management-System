...
func (s *UserServer) RegisterUserr(ctx context.Context, req *genuser.RegisterReq) (*genuser.RegisterResp, error) {
	...
	code, err := email.GenerateRandomCode()
	if err != nil {
		log.Fatalf("Failed to generate code: %v", err)
	}

	log.Println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>..")
	err = kafkaa.ProduceRegistrationEmail(req.Email, "VERF CODE", code)
	if err != nil {
		return nil, err
	}
...
func (s *UserServer) VerifyUserr(ctx context.Context, req *genuser.VerifyReq) (*genuser.VerifyResp, error) {
	...
	token, time, err := jwt.CreateToken(req.Email)
	if err != nil {
		log.Println("Error on createtoken", err)
		return nil, err
	}
	kafkaa.ProduceRegistrationEmail(userData["Email"], "Registration", "siz muvafaqiyatli royxatoan otingiz")
	return &genuser.VerifyResp{
		UserId:     userID,
		Token:      token,
		Message:    "User verified successfully.",
		ExpireTime: int64(time),
	}, nil
}
